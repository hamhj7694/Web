...
try {
    ...
    $current_my_like_count = intval($likes_json[$user_key] ?? 0);
    $next_my_like_count = $current_my_like_count + 1;

    $likes_json[$user_key] = $next_my_like_count;

    $next_likes_json = json_encode($likes_json, JSON_UNESCAPED_UNICODE);
    $next_total_like_count = intval($row['like_count']) + 1;
    ...
    if (!$update_stmt) {
        throw new Exception('추천 저장 준비 중 오류가 발생했어요.');
    }
    ...
    echo json_encode([
        ...
        'my_like_count' => $next_my_like_count
    ], JSON_UNESCAPED_UNICODE);
