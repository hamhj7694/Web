...
try {
    ...
    foreach ($replies as $index => $reply) {
        ...
        if (intval($reply['userNo'] ?? 0) !== $user_no) {
            throw new Exception('내가 작성한 의견만 수정할 수 있어요.');
        }

        $replies[$index]['text'] = $content;
        $replies[$index]['updatedAt'] = date('Y-m-d H:i:s');

        $updated_reply = $replies[$index];
        $found = true;
        break;
    }

    if (!$found) {
        throw new Exception('수정할 의견을 찾을 수 없어요.');
    }
    ...
    if (!$update_stmt) {
        throw new Exception('의견 수정 준비 중 오류가 발생했어요.');
    }
